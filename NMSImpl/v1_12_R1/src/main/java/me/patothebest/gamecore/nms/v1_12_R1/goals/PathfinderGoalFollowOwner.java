package me.patothebest.gamecore.nms.v1_12_R1.goals;
...
public class PathfinderGoalFollowOwner extends PathfinderGoal {
    ...
    @Override
    public boolean b() {
        return !this.navigation.o() && this.entity.h(this.enityOwner) > (double) (2.0f * 2.0f);
    }
        ...
        if (--this.someNumber <= 0) {
            ...
            if (!this.navigation.a(this.enityOwner, 1.0D)) {
                if (this.entity.h(this.enityOwner) >= 144.0D) {
                    ...
                    for (int x = 0; x <= 4; ++x) {
                        for (int z = 0; z <= 4; ++z) {
                            if ((x < 1 || z < 1 || x > 3 || z > 3) && this.enityOwner.getWorld().getType(new BlockPosition(locationX + x, boundingBox - 1, locationZ + z)).p() && this.a(new BlockPosition(locationX + x, boundingBox, locationZ + z)) && this.a(new BlockPosition(locationX + x, boundingBox + 1, locationZ + z))) {
    ...
    private boolean a(BlockPosition var1) {
        ...
        return var2.getMaterial() == Material.AIR || !var2.g();
    }
}