        ...
        if (entityShulker.world.getDifficulty() != EnumDifficulty.PEACEFUL) {
            ...
            if (var2 < 400.0D) {
                if (this.b <= 0) {
                    ...
                    EntityShulkerBullet var4 = new EntityShulkerBullet(entityShulker.world, entityShulker, var1, entityShulker.eq().m());
                    entityShulker.world.addEntity(var4);
                    entityShulker.a(SoundEffects.ENTITY_SHULKER_SHOOT, 2.0F, (random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F);
                }
            } else {
